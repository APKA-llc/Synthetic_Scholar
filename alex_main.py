...
COLLEGE_DIRECTORY = "Colleges/GT"
COLLEGE_TOPICS = "TEST_TOPICS.txt"      # TOPICS: "OFFICIAL_TOPICS.txt" and "TEST_TOPICS.txt"

pdfmetrics.registerFont(TTFont('Raleway', 'Fonts/Raleway-Medium.ttf'))
            ...
            for prompt_index in range(0, NUM_OF_PROMPTS):
                ...
                if "CS" or "STA" in current_subject:
                    prompts = [
                        ...
                        "Imagine you are a student studying for your final exams. Write a long and detailed list of sample computer science exam problems on " + current_topic + " for the course " + current_subject + " with solutions included. For every coding solution, start with a comment saying 'Start of Code' and end it with a comment saying 'End of Code'. Randomize the formatting.",
...
def generate_pdf(text, subject, topic, prompt_index):
    ...
    SimpleDocTemplate(COLLEGE_DIRECTORY + "/" + SUBFOLDER_DIRECTORY + "/" + DOC_TYPE + "-" + subject.upper() + "-" + topic.upper() + ".pdf", pagesize=letter, rightMargin=50.5,
                      leftMargin=40.5, topMargin=40.5,
                      bottomMargin=40.5).build(document)
